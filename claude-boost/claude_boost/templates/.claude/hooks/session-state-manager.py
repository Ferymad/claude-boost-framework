...
def run_command(cmd):
    """Run command and return output - secure version with command whitelisting"""
    try:
        # Split command properly for security
        if isinstance(cmd, str):
            cmd_parts = cmd.split()
        else:
            cmd_parts = cmd
        
        # Security: Command whitelisting - only allow safe git commands
        allowed_commands = ['git']
        if not cmd_parts or cmd_parts[0] not in allowed_commands:
            return ""
        
        # Additional git command validation
        if cmd_parts[0] == 'git':
            # Only allow safe read-only git operations
            safe_git_commands = ['log', 'branch', 'status', 'diff']
            if len(cmd_parts) < 2 or cmd_parts[1] not in safe_git_commands:
                return ""
...
def get_recent_activity():
    ...
    # Get current branch
    activity['current_branch'] = run_command(["git", "branch", "--show-current"])
    
    # Get git status
    activity['git_status'] = run_command(["git", "status", "--porcelain"])
